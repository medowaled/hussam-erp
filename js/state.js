            ...
            case STORAGE_KEYS.CUSTOMERS: {
                ...
                localList.forEach(localC => {
                    ...
                    if (!remoteC) {
                        ...
                        const localTime = Number(localC._updatedAt || 0);
                        const remoteTime = Number(remoteC._updatedAt || 0);

                        if (localTime > remoteTime && localTime > 0) {
                            mergedMap.set(idStr, { ...remoteC, ...localC });
                        } else if (remoteTime > localTime && remoteTime > 0) {
                            mergedMap.set(idStr, { ...localC, ...remoteC });
                        } else if (localPaid > remotePaid) {
                            mergedMap.set(idStr, { ...remoteC, ...localC });
                        } else if (remotePaid > localPaid) {
                            mergedMap.set(idStr, { ...localC, ...remoteC });
                        } else if (localPurchases > remotePurchases) {
                            mergedMap.set(idStr, { ...remoteC, ...localC });
                        } else if (Number(localC.debt) !== Number(remoteC.debt)) {
                            // If debt was explicitly edited, take the lower or newer
                            mergedMap.set(idStr, { ...localC, ...remoteC });
                        }
                    }
                });
